window.addEventListener("load",function(){
	...
	var requestURL="/portfolio/portfolio_detail_6/data/gnb.json";
	...
	request.addEventListener("load", function(){
		...
		function triggerScroll() {
			var timer=setTimeout(function(){
				...
				else if(scrollTop <= contentsLi[1].offsetTop){ //aboutTop 가기전
					n=1;
					remote.classList.add("active");
				}
				else if(scrollTop <= (contentsLi[3].offsetTop - 400)){ //detailTop 후 contactTop-400(메뉴와 좀 띄우기) 가기전
					n=2;
					remote.classList.add("active");
				}
				else { 
		...
		function clickMoving(evt){
			...
			if(n == 2) { //상단 좀 400 띄워줌 detail일 때만
	...
	function resizeFn(){
		for(var i=offsetYData.length; i>0; i--){ //리사이즈시 배열 초기화
			offsetYData.pop();
			scaleData.pop();
		}
	...
	window.addEventListener("scroll", function(){
		...
		frameTimer=setTimeout(function(){
			...
			else if(frameWindowTop >= offsetYData[1] && frameWindowTop < offsetYData[2]){
				count=1;
			}
			else if(frameWindowTop >= offsetYData[2] && frameWindowTop < offsetYData[3]){
				count=2;
			}
			else if(frameWindowTop >= offsetYData[3] && frameWindowTop < offsetYData[4]){
				count=3;
			}
			else if(frameWindowTop >= offsetYData[4] && frameWindowTop < offsetYData[5]){
				count=4;
			}
			else if(frameWindowTop >= offsetYData[5] && frameWindowTop < offsetYData[6]){
				count=5;
			}
			else if(frameWindowTop >= offsetYData[6] && frameWindowTop < offsetYData[7]){
				count=6;
			}
			else if(frameWindowTop >= offsetYData[7] && frameWindowTop < offsetYData[8]){
				count=7;
			}
			else if(frameWindowTop >= offsetYData[8] && frameWindowTop < offsetYData[9]){
				count=8;
			}
			else if(frameWindowTop >= offsetYData[9] && frameWindowTop < offsetYData[10]){
				count=9;
			}
			else if(frameWindowTop >= offsetYData[10]){
				count=10;
			}
